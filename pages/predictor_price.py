...
import bz2
import numpy as np
from functools import cache  # Import caching function

@cache(allow_none=True)  # Allow caching None values
def load_model():
...
def main():
    df, pipeline = load_model()  # Load model and pipeline (cached)
    ...
    if st.button('Predict'):
        data = [[property_type, sector, bedroom, bathroom, balcony,
                 ...
                 furnishing, luxury_category, floor_category]]
